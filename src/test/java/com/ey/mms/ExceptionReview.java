...
@CucumberOptions(features = "src/test/resources/features/Exception/exception_review.feature")
public class ExceptionReview {
	@BeforeClass
	public static void setup() {
		System.out.println("I'm in Exception Review \n");

		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");
		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "exceptionReviewBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "exceptionReviewMgmtBackup");
