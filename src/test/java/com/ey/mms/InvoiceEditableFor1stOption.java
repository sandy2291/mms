...
@CucumberOptions(features = "src/test/resources/features/Voiding/resubmission_for_first_opt.feature")
public class InvoiceEditableFor1stOption {
	@BeforeClass
	public static void setup() {
		System.out.println("I'm in Invoice Editable first option \n");

		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");

		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "ResubmissionFor1stOptBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "ResubmissionFor1stOptMgmt");
