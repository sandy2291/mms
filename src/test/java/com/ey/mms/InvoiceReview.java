...
@CucumberOptions(features = "src/test/resources/features/Invoice/invoice_review.feature")
public class InvoiceReview {
	@BeforeClass
	public static void setup() {
		System.out.println("I'm in Invoice Review");

		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");
		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "invoiceReviewBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "invoiceReviewMgmtBackup");
