...
@CucumberOptions(features="src/test/resources/features/Rejection/rejection_tracking_filter.feature")
public class RejectionTrackingFilter{

	/*@BeforeClass
	public static void setup() {
		System.out.println("I'm in Billing Contacts \n");
		
String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		
		String dbName = new PropertyReader().readProperty("dbName");
		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename,"AddColumnUpdateDateBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename,"AddColumnUpdateDateMgmtBackup");
