...
@CucumberOptions(features = "src/test/resources/features/Billing/upload_dx.feature")
public class UploadDx {
	@BeforeClass
	public static void setup() {
		System.out.println("I'm in upload DX \n");
		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");
		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "uploadDXBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "uploadDXMgmtBackup");
