...
@CucumberOptions(features = "src/test/resources/features/Fleet/uploadfleet.feature")
public class UploadFleet {
	@BeforeClass
	public static void setup() {
		System.out.println("I'm in upload fleet \n");
		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");
		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "uploadFleetBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "uploadFleetMgmtBackup");
