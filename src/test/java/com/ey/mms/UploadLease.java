...
@CucumberOptions(features = "src/test/resources/features/Leases/upload_lease.feature")
public class UploadLease {
	@BeforeClass
	public static void setup() {
		System.out.println("I'm in Upload lease");

		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");
		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "uploadLeaseBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "uploadLeaseMgmtBackup");
