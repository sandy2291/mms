...
@CucumberOptions(features = "src/test/resources/features/Voiding/mms_invoice_history_void.feature")
public class VoidingForOptions {

	@BeforeClass
	public static void setup() {
		System.out.println("I'm in Voiding for options \n");

		String dbbackupfilename = new PropertyReader().readProperty("dbbackup");
		String dbName = new PropertyReader().readProperty("dbName");

		String backup = EY_FW_Utility.getConfigProperty(dbbackupfilename, "voidingBackup");
		String dbNameMgmt = new PropertyReader().readProperty("dbNameMgmt");
		String backupMgmt = EY_FW_Utility.getConfigProperty(dbbackupfilename, "voidingMgmtBackup");
